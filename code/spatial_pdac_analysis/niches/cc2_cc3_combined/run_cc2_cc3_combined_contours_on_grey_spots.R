...
###############################################################################
# Combined CC2+CC3 contours on grey spot maps
#
# This script shows all spots in grey and outlines the merged CC2+CC3 niche.
# CC2 and CC3 are treated as one group before connected components and concave
# hulls are computed.
###############################################################################
...
message("Loading ST Seurat object for combined CC2+CC3 contour maps...")
...
write_tsv_simple(bind_rows(component_summary), "cc2_cc3_combined_contour_component_summary.tsv", tab_dir)

save_multipage_pdf(
  plot_list = plot_list,
  filename = "cc2_cc3_combined_contours_on_grey_spots_normal_and_primary.pdf",
  out_dir = fig_dir,
  width = 4.0,
  height = 3.2
)
...
message("ST PDAC combined CC2+CC3 contour maps complete.")
