...
# CC5 contours only
#
# This script makes a very simple version of the CC5 contour maps with no spot
# colors, only thin black contours on a white background.
###############################################################################
...
build_contours_only_plot <- function(seurat_obj, image_name, metadata_df) {
...
message("Loading ST Seurat object for CC5 contour-only maps...")
...
write_tsv_simple(selected_images, "selected_normal_and_primary_sections_for_cc5_contours_only.tsv", tab_dir)
...
for (img_name in selected_images$orig.ident) {
  ...
  plot_list[[img_name]] <- build_contours_only_plot(
    seurat_obj = st_obj,
    image_name = img_name,
...
write_tsv_simple(bind_rows(component_summary), "cc5_contour_components_only.tsv", tab_dir)

save_multipage_pdf(
  plot_list = plot_list,
  filename = "cc5_contours_only_normal_and_primary.pdf",
  out_dir = fig_dir,
  width = 4.0,
  height = 3.2
)
...
message("ST PDAC CC5 contour-only maps complete.")
