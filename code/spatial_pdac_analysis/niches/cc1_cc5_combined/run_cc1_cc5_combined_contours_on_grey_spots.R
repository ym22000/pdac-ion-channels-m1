...
###############################################################################
# Combined CC1+CC5 contours on grey spot maps
#
# This script shows all spots in grey and outlines the merged CC1+CC5 niche.
# CC1 and CC5 are treated as one group before connected components and concave
# hulls are computed.
###############################################################################
...
build_combined_contour_plot <- function(seurat_obj, image_name, metadata_df) {
...
message("Loading ST Seurat object for combined CC1+CC5 contour maps...")
...
for (img_name in selected_images$orig.ident) {
  ...
  plot_list[[img_name]] <- build_combined_contour_plot(
    seurat_obj = st_obj,
    image_name = img_name,
...
write_tsv_simple(bind_rows(component_summary), "cc1_cc5_combined_contour_component_summary.tsv", tab_dir)

save_multipage_pdf(
  plot_list = plot_list,
  filename = "cc1_cc5_combined_contours_on_grey_spots_normal_and_primary.pdf",
  out_dir = fig_dir,
  width = 4.0,
  height = 3.2
)
...
message("ST PDAC combined CC1+CC5 contour maps complete.")
