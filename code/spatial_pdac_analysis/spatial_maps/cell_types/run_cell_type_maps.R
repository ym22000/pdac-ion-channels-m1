...
###############################################################################
# Spatial maps colored by dominant spot identity
#
# This script uses `first_type` to show the main dominant spot identity on the
# same set of normal and primary sections used in the ecotype maps.
###############################################################################
...
build_spatial_category_plot <- function(seurat_obj, image_name, metadata_df, group_var, palette_values, point_size = 0.65) {
...
build_manual_color_legend <- function(color_mapping_df, title_text = "first_type", label_col = "label") {
  ...
  ggplot(legend_df, aes(x = 1, y = y)) +
    ...
    guides(fill = "none") +
    coord_cartesian(xlim = c(0.7, 4.9), ylim = c(0.5, nrow(legend_df) + 0.8), clip = "off") +
...
message("Loading ST Seurat object for first_type spatial panels...")
...
first_type_color_mapping <- data.frame(
  label = first_type_levels,
  color = unname(first_type_colors[first_type_levels]),
  stringsAsFactors = FALSE
)
write_tsv_simple(first_type_color_mapping, "cell_type_color_mapping.tsv", tab_dir)
...
panel_list <- lapply(selected_images$orig.ident, function(image_name) {
  spatial_plot <- build_spatial_category_plot(
    ...
    group_var = "first_type",
    palette_values = first_type_colors,
    point_size = 0.65
  )

  spatial_plot + build_manual_color_legend(
    color_mapping_df = first_type_color_mapping,
    title_text = "first_type",
    label_col = "label"
...
save_multipage_pdf(
  ...
  filename = "normal_and_primary_cell_type_maps.pdf",
  out_dir = fig_dir,
  width = 4.4,
  height = 3.6
)
...
message("ST PDAC first_type spatial panels complete.")
