...
###############################################################################
# KCN expression maps with combined CC1+CC5 contours
#
# This script reuses the standard KCN projection maps and adds the combined
# CC1+CC5 contour on top. The contour is drawn as one merged niche based on the
# connected CC1 and CC5 spot islands.
###############################################################################
...
compute_component_hulls <- function(coords_df, target_labels = c("CC1", "CC5")) {
  cc_df <- coords_df %>%
    filter(cc_ischia_10 %in% target_labels) %>%
...
build_spatial_expression_combined_plot <- function(
    seurat_obj,
    image_name,
    gene_df,
    metadata_df,
  ...
  hull_df <- compute_component_hulls(coords_df, target_labels = c("CC1", "CC5"))
...
message("Loading ST Seurat object for KCN25 projections with combined CC1+CC5 contours...")
...
expression_palette <- grDevices::colorRampPalette(RColorBrewer::brewer.pal(9, "PuRd"))(100)
...
for (gene_name in kcn_list) {
  ...
  plot_list <- lapply(selected_images$orig.ident, function(img_name) {
    build_spatial_expression_combined_plot(
      seurat_obj = st_obj,
      image_name = img_name,
      gene_df = gene_df,
  ...
  save_multipage_pdf(
    plot_list = plot_list,
    filename = paste0("", gene_name, "_expression_with_cc1_cc5_contours.pdf"),
...
write_tsv_simple(sample_summary_all, "kcn_expression_projection_with_cc1_cc5_contours_by_section.tsv", tab_dir)
write_tsv_simple(ecotype_summary_all, "kcn_expression_projection_with_cc1_cc5_contours_by_ecotype.tsv", tab_dir)


message("KCN25 expression projections with combined CC1+CC5 contours complete.")
