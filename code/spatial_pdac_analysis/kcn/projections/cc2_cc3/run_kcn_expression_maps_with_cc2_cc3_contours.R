...
###############################################################################
# KCN expression maps with combined CC2+CC3 contours
#
# This script reuses the standard KCN projection maps and adds the combined
# CC2+CC3 contour on top. The contour is drawn as one merged niche based on the
# connected CC2 and CC3 spot islands.
###############################################################################
...
compute_component_hulls <- function(coords_df, target_labels = c("CC2", "CC3")) {
  cc_df <- coords_df %>%
...
build_spatial_expression_combined_plot <- function(
  ...
  hull_df <- compute_component_hulls(coords_df, target_labels = c("CC2", "CC3"))
...
message("Loading ST Seurat object for KCN25 projections with combined CC2+CC3 contours...")
...
for (gene_name in kcn_list) {
  ...
  save_multipage_pdf(
    plot_list = plot_list,
    filename = paste0("", gene_name, "_expression_with_cc2_cc3_contours.pdf"),
...
write_tsv_simple(sample_summary_all, "kcn_expression_projection_with_cc2_cc3_contours_by_section.tsv", tab_dir)
write_tsv_simple(ecotype_summary_all, "kcn_expression_projection_with_cc2_cc3_contours_by_ecotype.tsv", tab_dir)


message("KCN25 expression projections with combined CC2+CC3 contours complete.")
