...
pdf_path = REPORT_DIR / "kcn_ecotype_by_section_primary.pdf"

rows_per_page = 24
legend_lines = [
    "Par KCN: ligne 1 = CC1+CC5, ligne 2 = CC2+CC3.",
...
with PdfPages(pdf_path) as pdf:
    for section_name in sorted(display_df["Section"].unique()):
        ...
        # Rank KCN within each section by best FDR across the two ecotype contrasts.
        section_rank_df = (
            report_df[report_df["Section"] == section_name]
            .groupby("KCN", as_index=False)
            .agg(
                min_fdr=("FDR", "min"),
                max_abs_log2fc=("abs_log2fc", "max"),
            )
            .sort_values(
                ["min_fdr", "max_abs_log2fc", "KCN"],
                ascending=[True, False, True],
            )
            .reset_index(drop=True)
        )
        kcn_order = {kcn: idx for idx, kcn in enumerate(section_rank_df["KCN"])}
        section_df["kcn_order"] = section_df["KCN"].map(kcn_order)
        section_df = section_df.sort_values(["kcn_order", "contrast_order"]).reset_index(drop=True)

        previous_kcn = None
        for idx in section_df.index:
            current_kcn = section_df.at[idx, "KCN"]
            if current_kcn == previous_kcn:
                section_df.at[idx, "KCN"] = ""
        ...
        for page_idx in range(n_pages):
            ...
            page_df = section_df.iloc[start:end].drop(columns=["Section", "contrast_order", "abs_log2fc", "kcn_order"])

            fig = plt.figure(figsize=(16.5, 11.7))
            ax = fig.add_axes([0.03, 0.20, 0.94, 0.72])
            ax.axis("off")

            title = f"KCN ecotype summary by section: {section_name}"
            if n_pages > 1:
