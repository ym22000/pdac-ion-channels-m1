...
###############################################################################
# KCN ecotype-vs-rest contrasts in primary PDAC sections
#
# This script replaces the niche-level pooled Fisher approach with a
# section-wise expression contrast. For each primary PDAC section and each KCN,
# expression is compared between one ecotype group and all remaining spots.
#
...
#
# Output metrics:
# - number of spots inside/outside the contrast
# - fraction of positive spots
# - mean expression (all spots)
# - mean expression among positive spots
# - log2 fold change based on mean expression
# - Wilcoxon p-value and FDR
# - AUC derived from the Wilcoxon rank-sum statistic
###############################################################################
...
message("Loading ST Seurat object for KCN ecotype-vs-rest contrasts...")
...
write_tsv_simple(results_df, "kcn_ecotype_vs_rest_by_section.tsv", tab_dir)
...
write_tsv_simple(summary_df, "kcn_ecotype_vs_rest_summary.tsv", tab_dir)

preferred_niche_df <- summary_df %>%
  select(
    gene, contrast, median_log2fc_mean_expression, median_auc,
    n_sections_fdr_lt_0_05, median_delta_pct_positive
  ) %>%
  pivot_wider(
    names_from = contrast,
    values_from = c(
  ...
  mutate(
    favored_contrast_by_log2fc = case_when(
      median_log2fc_mean_expression__CC1_CC5 > median_log2fc_mean_expression__CC2_CC3 ~ "CC1_CC5",
      median_log2fc_mean_expression__CC2_CC3 > median_log2fc_mean_expression__CC1_CC5 ~ "CC2_CC3",
      TRUE ~ "tie"
    ),
    favored_contrast_by_auc = case_when(
      median_auc__CC1_CC5 > median_auc__CC2_CC3 ~ "CC1_CC5",
      median_auc__CC2_CC3 > median_auc__CC1_CC5 ~ "CC2_CC3",
      TRUE ~ "tie"
    )
  )

write_tsv_simple(preferred_niche_df, "kcn_preferred_ecotype_contrast_primary.tsv", tab_dir)


message("KCN ecotype-vs-rest contrasts complete.")
