...
###############################################################################
# KCNN4 pathway overlay in primary PDAC sections
#
# Goal:
#   Reproduce the logic used in the reference PDAC paper for pathway activity
#   maps, but focus on one question: with which continuous pro-tumoral program
#   does KCNN4 spatially overlap in primary PDAC?
#
# Strategy:
#   1. Compute pathway activity scores per spot with `escape::runEscape()`.
#   2. Standardize each pathway score across primary PDAC spots.
#   3. Rebuild the KCNN4 Hotspot view from the exported raw-count sections.
#   4. Save one PDF per pathway with a double panel:
#      left  = KCNN4 Hotspot-style expression map
#      right = pathway activity map in a magma palette
#
# The script keeps the code simple and explicit on purpose so it is easy to
# rerun and adapt later in the project.
###############################################################################
...
PATHWAY_GENE_SETS <- list(
  ductal_tumoral_epithelial = c(
    "EPCAM", "KRT8", "KRT18", "KRT7", "CLDN4", "MUC1", "SOX9", "ELF3",
    "TACSTD2", "TSPAN8"
  ),
  tumor_stress_secretory = c(
    "LCN2", "SLPI", "S100A10", "S100P", "MMP7", "SPINK1", "CEACAM5",
    "MSLN", "KRT20", "KRT23"
  ),
  Tumor_proliferation_rate = c(
    "AURKA", "AURKB", "BUB1", "CCNB1", "CCND1", "CCNE1", "CDK2", "CETN3",
    "E2F1", "ESCO2", "MCM2", "MCM6", "MKI67", "MYBL2", "PLK1"
  )
)
...
safe_update_seurat <- function(seurat_obj) {
  # The object already loads correctly with current Seurat for the operations
  # used here (subset, assay access, escape scoring). Skipping a full update
  # keeps the rerun much faster and avoids rewriting every image slot.
  seurat_obj
}
...
load_section_payload <- function(section_dir, target_gene) {
  ...
  if (!"spot" %in% colnames(coords_df) && ncol(coords_df) >= 1) {
    # Some exported TSV files can carry a BOM or a slightly altered first
    # column name on Windows; here we force the first column back to `spot`.
...
make_section_map <- function(exports_dir, section_dirs) {
...
make_summary_page <- function(pathway_label, gene_label, gene_set_text, corr_df) {
  ...
  top_plot <- ggplot(corr_df, aes(x = reorder(orig.ident, spearman_rho), y = spearman_rho)) +
    ...
    labs(
      ...
      subtitle = "Spearman correlation between KCNN4 raw counts and scaled pathway score",
      x = NULL,
      y = "Spearman rho"
    ) +
...
if (!file.exists(input_object_path)) {
  stop("Missing Seurat object: ", input_object_path)
}
if (!dir.exists(exports_dir)) {
  stop("Missing exports directory: ", exports_dir)
}
if (!file.exists(hotspot_table_path)) {
  stop("Missing Hotspot section table: ", hotspot_table_path)
}
if (!requireNamespace("escape", quietly = TRUE)) {
  stop("Package 'escape' is required. Please install it before rerunning this script.")
}
...
section_map <- make_section_map(exports_dir, section_dirs)
...
write_tsv_simple(hotspot_stats, file.path(tab_dir, "kcnn4_hotspot_stats_by_section.tsv"))
...
if (!"RNA" %in% names(st_obj@assays)) {
  # The reference paper works from an RNA assay, but this spatial object only
  # stores a Spatial assay. We expose the same matrix under an RNA alias so the
  # escape API behaves as expected on this object.
...
message("Computing section-wise overlap between KCNN4 and pathway scores...")
...
write_tsv_simple(correlation_df, file.path(tab_dir, "kcnn4_pathway_correlations_by_section.tsv"))
write_tsv_simple(correlation_summary, file.path(tab_dir, "kcnn4_pathway_correlations_summary.tsv"))
...
message("Rendering double-panel PDFs...")
for (pathway_name in names(PATHWAY_GENE_SETS)) {
  pathway_scores_this <- score_long_df %>%
  ...
  pathway_corr_this <- correlation_df %>%
    filter(pathway == pathway_name)

  pdf_path <- file.path(
    fig_dir,
      paste0(tolower(pathway_name), "_kcnn4_hotspot_vs_pathway_primary.pdf")
  )
  ...
  for (section_name in names(section_payloads)) {
    ...
    hotspot_row <- hotspot_stats %>%
      filter(orig.ident == section_name)
    corr_row <- pathway_corr_this %>%
      filter(orig.ident == section_name)
...
message("KCNN4 pathway overlay primary run complete.")
