#!/usr/bin/env python
"""
Hotspot local correlation analysis for KCN genes in normal pancreas sections.

This script keeps the same logic as the primary PDAC local-correlation run, but
it is restricted to the three normal pancreas sections. The idea is to ask
which genes repeatedly follow the same local spatial pattern as each KCN in a
healthy tissue context.
"""
...
EXPORTS_DIR_DEFAULT = SCRIPT_DIR.parent / "exports_normal_pancreas" / "exports"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run section-wise Hotspot local correlations for all KCN genes in normal pancreas."
    )
    parser.add_argument(
        "--exports-dir",
...
def save_target_pdf(
    ...
    with PdfPages(pdf_path) as pdf:
        ...
        ax.set_title(f"{target_gene} local correlation Z-scores across normal sections")
...
def main() -> None:
    ...
    for section_dir in section_dirs:
        ...
        print(f"[Hotspot normal local corr] Processing {image_name}...")
        try:
    ...
    for target_gene in target_genes:
        ...
        save_target_pdf(
            ...
            pdf_path=fig_dir / f"{target_gene.lower()}_local_correlations_normal_pancreas_sections.pdf",
        )
