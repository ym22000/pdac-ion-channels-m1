...
###############################################################################
# KCNMA1 EMT Hallmark overlay in primary PDAC sections
#
# Goal:
#   Reproduce the logic used in the reference PDAC paper for pathway activity
#   maps, but focus on one question: does KCNMA1 spatially overlap with the
#   MSigDB Hallmark EMT program?
#
# Strategy:
...
#   3. Rebuild the KCNMA1 Hotspot view from the exported raw-count sections.
#   4. Save one PDF per pathway with a double panel:
#      left  = KCNMA1 Hotspot-style expression map
...
EMT_HALLMARK_NAME <- "HALLMARK_EPITHELIAL_MESENCHYMAL_TRANSITION"
EMT_DISPLAY_NAME <- "EMT"
...
make_summary_page <- function(pathway_label, gene_label, gene_set_text, corr_df) {
  ...
  top_plot <- ggplot(corr_df, aes(x = reorder(orig.ident, spearman_rho), y = spearman_rho)) +
    ...
    labs(
      ...
      subtitle = "Spearman correlation between KCNMA1 raw counts and scaled pathway score",
      x = NULL,
      y = "Spearman rho"
    ) +
...
png_dir <- file.path(fig_dir, "png_sections")
...
hallmark_tbl <- msigdbr(
  ...
  filter(gs_name == EMT_HALLMARK_NAME)

if (nrow(hallmark_tbl) == 0) {
  stop("Could not retrieve the EMT Hallmark gene set from msigdbr.")
}

PATHWAY_GENE_SETS <- list(
  EMT = unique(hallmark_tbl$gene_symbol)
)

if (file.exists(score_cache_path)) {
  score_long_df <- read.delim(score_cache_path, check.names = FALSE)
  if ("pathway" %in% colnames(score_long_df)) {
    score_long_df$pathway[score_long_df$pathway == EMT_HALLMARK_NAME] <- EMT_DISPLAY_NAME
  }
...
message("Computing section-wise overlap between KCNMA1 and pathway scores...")
...
for (pathway_name in names(PATHWAY_GENE_SETS)) {
  ...
  pdf_path <- file.path(
    fig_dir,
    paste0(tolower(pathway_name), "_kcnma1_hotspot_vs_pathway_primary.pdf")
  )
  ...
  for (section_name in names(section_payloads)) {
    ...
    combined_plot <- left_plot + right_plot + plot_layout(ncol = 2)
    print(combined_plot)

    png_path <- file.path(
      png_dir,
      paste0(
        tolower(pathway_name), "_kcnma1_hotspot_vs_pathway_primary_",
        section_name, ".png"
      )
    )
    ggsave(
...
message("KCNMA1 pathway overlay primary run complete.")
