#!/usr/bin/env python
"""
Hotspot spatial autocorrelation analysis for all KCN genes (Venn DEGs) in primary PDAC.

This script was added to test whether KCN genes show spatial structure inside
primary PDAC sections, not only enrichment in specific ecotypes or spot labels.
Some genes can be enriched in a niche but still look diffuse across the tissue.
Hotspot is useful here because it measures local spatial autocorrelation and
helps identify genes with patch-like spatial patterns.

The analysis is done section by section because spatial distances only make
sense within the same Visium slide. The workflow follows the standard Hotspot
spatial setup: raw counts, spatial coordinates, KNN graph, then gene-wise
autocorrelation. The script writes one PDF per gene together with simple summary
tables so the run is easy to reproduce and inspect.

Only primary sections are used here. The main goal is to describe spatial
structure in tumor tissue, where most of the KCN genes prioritized in this
project are the most informative. Some genes such as KCNJ8 also have signal in
normal tissue, but that is a separate question and would be better analyzed in
its own normal-only run instead of mixing biological contexts.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run section-wise Hotspot spatial autocorrelation for all KCN genes."
    )
    parser.add_argument(
        "--exports-dir",
...
def keep_genes_for_hotspot(adata: ad.AnnData, target_genes: list[str]) -> ad.AnnData:
    ...
    # Keep target genes when they are present, even if they are sparse.
    ...
    # Drop genes with zero variance to avoid Hotspot failures on some sections.
    x = adata.X
