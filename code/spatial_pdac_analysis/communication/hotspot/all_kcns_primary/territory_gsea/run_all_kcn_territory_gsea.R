...
###############################################################################
# Hallmark GSEA for KCN-high territories in primary PDAC sections
#
# For each KCN, this script defines a KCN-high territory inside each primary
# section, compares it to the rest of the same section, and then runs Hallmark
# GSEA on the average section-level expression difference.
#
...
# ranking genes than the Spatial data layer in this object.
###############################################################################
...
for (target_gene in kcn_genes) {
  ...
  figure_path <- file.path(fig_dir, paste0(tolower(target_gene), "_territory_hallmark_gsea.pdf"))
