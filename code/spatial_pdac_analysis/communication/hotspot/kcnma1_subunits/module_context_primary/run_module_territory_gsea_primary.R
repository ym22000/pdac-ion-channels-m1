...
TARGET_MODULES <- list(
  KCNMA1_KCNMB1 = c("KCNMA1", "KCNMB1"),
  KCNMA1_KCNMB4 = c("KCNMA1", "KCNMB4")
)
...
make_module_summary_row <- function(module_name, components, fgsea_res, gene_stats_df, section_summary_df) {
...
plot_fgsea_bubble <- function(gsea_res, target_module, file_path) {
  filtered <- gsea_res %>%
  ...
  if (nrow(plot_df) == 0) {
    ...
    plot.new()
    text(0.5, 0.5, paste0("No Hallmark pathway passed the plotting filters for ", target_module, "."))
    grDevices::dev.off()
  ...
  p <- ggplot(plot_df, aes(x = NES, y = pathway_label)) +
    ...
    labs(
      title = paste0(target_module, " territory Hallmark GSEA"),
...
plot_placeholder_page <- function(target_module, message_text, file_path) {
  grDevices::pdf(file_path, width = 10, height = 7)
  plot.new()
  title(main = paste0(target_module, " territory Hallmark GSEA"))
  text(
    x = 0.5,
    y = 0.58,
    labels = message_text,
...
message("Loading spatial object for module territory GSEA...")
...
availability_df <- bind_rows(lapply(names(TARGET_MODULES), function(module_name) {
  comps <- TARGET_MODULES[[module_name]]
  data.frame(
    module = module_name,
    components = paste(comps, collapse = "+"),
    all_components_in_sct = all(comps %in% rownames(target_mat)),
    stringsAsFactors = FALSE
  )
}))
write_tsv_simple(availability_df, "module_territory_gsea_availability.tsv", tab_dir)
...
all_gene_stats <- list()
all_fgsea <- list()
all_module_summaries <- list()

for (module_name in names(TARGET_MODULES)) {
  comps <- TARGET_MODULES[[module_name]]
  figure_path <- file.path(fig_dir, paste0(tolower(module_name), "_territory_hallmark_gsea.pdf"))

  if (!all(comps %in% rownames(target_mat))) next
  ...
  for (section_name in primary_sections) {
    ...
    module_expr <- Matrix::colSums(target_mat[comps, section_spots, drop = FALSE])
    names(module_expr) <- section_spots
    high_spots <- select_high_spots(module_expr)
    ...
    section_rows[[section_name]] <- data.frame(
      ...
      n_positive_spots = sum(module_expr > 0),
  ...
  all_section_summaries[[module_name]] <- section_summary_df

  if (length(gene_deltas) == 0) {
    plot_placeholder_page(
      target_module = module_name,
      message_text = "Too few primary sections passed the territory filters.\nNo territory GSEA could be run for this module.",
      file_path = figure_path
    )
    all_module_summaries[[module_name]] <- data.frame(
      module = module_name,
      components = paste(comps, collapse = "+"),
      n_sections_used = 0,
  ...
  all_gene_stats[[module_name]] <- gene_stats_df
  ...
  fgsea_res <- suppressWarnings(
    ...
    as.data.frame() %>%
    arrange(padj, desc(abs(NES))) %>%
    mutate(module = module_name, components = paste(comps, collapse = "+"))
  ...
  all_fgsea[[module_name]] <- fgsea_res

  all_module_summaries[[module_name]] <- make_module_summary_row(
...
if (length(all_section_summaries) > 0) write_tsv_simple(bind_rows(all_section_summaries), "module_territory_section_summary.tsv", tab_dir)
if (length(all_gene_stats) > 0) write_tsv_simple(bind_rows(all_gene_stats), "module_territory_gene_statistics.tsv", tab_dir)
if (length(all_fgsea) > 0) write_tsv_simple(bind_rows(all_fgsea), "module_territory_hallmark_fgsea.tsv", tab_dir)
if (length(all_module_summaries) > 0) write_tsv_simple(bind_rows(all_module_summaries), "module_territory_gsea_summary.tsv", tab_dir)

message("Module territory GSEA complete.")
