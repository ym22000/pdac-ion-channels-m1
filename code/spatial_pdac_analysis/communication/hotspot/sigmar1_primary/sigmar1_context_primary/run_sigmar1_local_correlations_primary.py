#!/usr/bin/env python3
"""
Hotspot local correlations for SIGMAR1 in primary PDAC.
"""
...
def extract_target_profile(local_corr_z: pd.DataFrame, image_name: str) -> pd.DataFrame:
    profile = (
...
def build_partner_summary(profile_df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for partner_gene, group in profile_df.groupby("partner_gene", sort=True):
        rows.append(
            {
    ...
    return pd.DataFrame(rows).sort_values(
        ["n_sections_positive_z", "median_local_corr_z"],
...
def main() -> None:
    ...
    forced_genes = [TARGET_GENE] + kcn_genes
    ...
    for section_dir in sorted([p for p in EXPORTS_DIR.iterdir() if p.is_dir()]):
        ...
        print(f"[SIGMAR1 local corr] Processing {image_name}...")
        adata = load_section(section_dir)
        adata = keep_genes_for_hotspot(adata, forced_genes)

        available = TARGET_GENE in adata.var_names
        availability_rows.append(
            {"target_gene": TARGET_GENE, "orig.ident": image_name, "available_for_local_corr": available}
        )
        if not available:
            continue
        ...
        if TARGET_GENE not in genes_for_corr:
            genes_for_corr = [TARGET_GENE] + genes_for_corr
            genes_for_corr = list(dict.fromkeys(genes_for_corr))
        local_corr_z = hs.compute_local_correlations(genes_for_corr, jobs=1)
        if TARGET_GENE not in local_corr_z.index:
            continue

        profile_rows.append(extract_target_profile(local_corr_z, image_name))
        stats = autocorr.loc[TARGET_GENE] if TARGET_GENE in autocorr.index else pd.Series({"C": np.nan, "Z": np.nan, "FDR": np.nan})
        ...
        pd.DataFrame(availability_rows).to_csv(TABLES_DIR / "_checkpoint_target_gene_availability.tsv", sep="\t", index=False)
        if profile_rows:
            pd.concat(profile_rows, ignore_index=True).to_csv(
                TABLES_DIR / "_checkpoint_local_correlations_by_target_and_section.tsv",
                sep="\t",
                index=False,
            )
        pd.DataFrame(section_summary_rows).to_csv(
            TABLES_DIR / "_checkpoint_target_gene_section_summary.tsv",
            sep="\t",
            index=False,
        )
    ...
    kcn_by_section.to_csv(TABLES_DIR / "sigmar1_kcn_local_correlations_by_target_and_section.tsv", sep="\t", index=False)
    kcn_summary.to_csv(TABLES_DIR / "sigmar1_kcn_local_correlations_summary.tsv", sep="\t", index=False)
    ...
    if not profile_df.empty and not summary_df.empty:
        save_target_pdf(
            target_section_df=profile_df,
            target_summary_df=summary_df,
            pdf_path=FIGURES_DIR / "sigmar1_local_correlations_primary_sections.pdf",
        )

    print("SIGMAR1 local correlations complete.")
