###############################################################################
# Bulk RNA-seq analysis for the BKCa / shBKCa dataset
#
# This script works on the local CTRL vs shBKCa bulk RNA-seq data. The goal is
# to get a clean summary of the perturbation, with QC plots, differential
# expression, CAF-oriented signatures, and pathway-level results.
#
# The analysis starts from a local gene-level table that was produced upstream,
# so DESeq2 is not rerun here.
###############################################################################
...
# Main input: local gene-level table with expression and DE statistics.
input_file <- file.path(input_dir, "Differential_expression_analysis_table-shBKCAshctrl.csv")
...
heatmap_scale_spec <- function(mat, n = 101) {
  ...
  if (rng[1] < 0 && rng[2] > 0) {
    ...
    return(list(
      colors = seismic_cols,
      breaks = seq(-lim, lim, length.out = length(seismic_cols) + 1),
  ...
  list(
    colors = heat_blues,
    breaks = seq(lower, upper, length.out = length(heat_blues) + 1),
    palette_name = "Blues"
  )
}
  ...
  draw_heatmap <- function() {
    ...
    label_grobs <- lapply(seq_along(block_labels), function(i) {
      grid::textGrob(
        ...
        rot = 90,
        gp = grid::gpar(fontsize = 7.4, fontface = "bold")
      )
    })
    gt <- gtable::gtable_add_grob(
      gt,
...
save_landscape_block_heatmap <- function(display_mat,
                                         annotation_col,
                                         annotation_colors,
                                         gaps_row,
                                         file_path,
...
save_replicate_gene_landscape_heatmap <- function(display_mat,
                                                  annotation_row,
                                                  annotation_colors,
                                                  gaps_row,
                                                  gaps_col,
                                                  ...
                                                  height = 4.2,
                                                  title_text = "CAF signature heatmap") {
  heat_cols <- grDevices::colorRampPalette(c("#2166ac", "white", "#b2182b"))(101)
  lim <- max(abs(display_mat), na.rm = TRUE)
  if (!is.finite(lim) || lim == 0) lim <- 1
  heat_breaks <- seq(-lim, lim, length.out = length(heat_cols) + 1)
  ...
  draw_heatmap <- function() {
    ...
    matrix_r <- gt$layout$r[matrix_idx]
    matrix_t <- gt$layout$t[matrix_idx]
    gt <- gtable::gtable_add_rows(gt, heights = grid::unit(1.0, "cm"), pos = 0)
    gt <- gtable::gtable_add_grob(
      gt,
      grobs = grid::textGrob(title_text, gp = grid::gpar(fontsize = 13, fontface = "bold")),
      t = 1, l = matrix_l, r = matrix_r, clip = "off", name = "landscape_title"
    )
    grid::grid.newpage()
    grid::grid.draw(gt)
  }
...
condition_from_sample <- function(x) {
  ifelse(grepl("^sushi66", x), "shBKCa", "CTRL")
}
...
plot_density_lines <- function(mat, phenotype, file_path) {
  ...
  legend("topright", legend = c("CTRL", "shBKCa"), col = c(ctrl_color, sh_color), lwd = 2, bty = "n")
  dev.off()
}
...
plot_qc_boxplots <- function(mat, phenotype, file_path) {
  ...
  legend("topright", legend = c("CTRL", "shBKCa"), fill = c(ctrl_color, sh_color), bty = "n")
  dev.off()
}
...
plot_pca <- function(mat, phenotype, file_path) {
  ...
  p <- ggplot(pca_df, aes(x = PC1, y = PC2, fill = condition, label = sample)) +
    geom_point(shape = 21, size = 4.1, stroke = 0.4, colour = "black") +
    geom_text(vjust = -1.05, size = 3.4) +
    scale_fill_manual(values = c("CTRL" = ctrl_color, "shBKCa" = sh_color), name = "Condition") +
    labs(
      title = "PCA of bulk RNA-seq replicates",
      x = paste0("PC1 (", percent[1], "%)"),
      y = paste0("PC2 (", percent[2], "%)")
    ) +
    theme_stage(base_size = 12) +
    theme(
      axis.text.x = element_text(angle = 0, hjust = 0.5),
      legend.title = element_text(face = "bold")
    )
  save_plot(p, file_path, 7, 6)
  pca_df$variance_PC1 <- percent[1]
  pca_df$variance_PC2 <- percent[2]
  pca_df$variance_PC3 <- percent[3]
  pca_df
}
...
plot_volcano <- function(res_df, file_path) {
  ...
  plot_df$class <- "NS"
  plot_df$class[plot_df$padj < 0.05 & plot_df$log2FoldChange > 1] <- "Up in shBKCa"
  plot_df$class[plot_df$padj < 0.05 & plot_df$log2FoldChange < -1] <- "Down in shBKCa"
  cols <- c("Down in shBKCa" = ctrl_color, "NS" = neutral_color, "Up in shBKCa" = sh_color)
  ...
  labs_df <- unique(rbind(
    ...
    plot_df[plot_df$gene_symbol %in% c("KCNMA1", "ACTA2", "IL6", "C3", "CD74"), , drop = FALSE]
  ))
  p <- ggplot(plot_df, aes(x = log2FoldChange, y = y, colour = class)) +
    geom_point(size = 1.3, alpha = 0.9) +
    geom_vline(xintercept = c(-1, 1), linetype = 2, linewidth = 0.35, colour = "grey55") +
    geom_hline(yintercept = -log10(0.05), linetype = 2, linewidth = 0.35, colour = "grey55") +
    scale_colour_manual(values = cols, name = NULL) +
    labs(
      title = "Volcano plot - shBKCa vs CTRL",
      x = "log2 Fold Change (shBKCa / CTRL)",
      y = "-log10(FDR)"
    ) +
    theme_stage(base_size = 12) +
    theme(
  ...
  if (nrow(labs_df) > 0) {
    p <- p + ggrepel::geom_text_repel(
      data = labs_df,
      aes(x = log2FoldChange, y = y, label = gene_symbol),
      inherit.aes = FALSE,
      size = 3,
      box.padding = 0.35,
      point.padding = 0.2,
...
plot_ma_like <- function(res_df, file_path) {
  ...
  plot(res_df$mean_expression, res_df$log2FoldChange,
       ...
       xlab = "Mean log2 expression", ylab = "log2 Fold Change (shBKCa / CTRL)",
       main = "MA-like plot")
  abline(h = 0, lty = 2, col = "grey40")
  legend("topright", legend = c("Down in shBKCa", "NS", "Up in shBKCa"),
...
plot_marker_violin_panels <- function(expr_mat, phenotype, genes, marker_stats, file_path) {
  ...
  for (g in genes) {
    ...
    vals_sh <- expr_mat[g, phenotype$condition == "shBKCa"]
    ...
    plot(NA, xlim = c(0.5, 2.5), ylim = ylim, xaxt = "n", xlab = "", ylab = "log2(count + 1)",
         main = g, cex.main = 1)
    axis(1, at = c(1, 2), labels = c("CTRL", "shBKCa"))
...
plot_signature_score_panels <- function(score_mat, phenotype, stats_df, file_path) {
  ...
  for (sig in sigs) {
    ...
    x_sh <- score_mat[phenotype$condition == "shBKCa", sig]
    y_all <- c(x_ctrl, x_sh)
    ylim <- range(y_all, na.rm = TRUE)
    ...
    if (nrow(st) == 1) {
      ...
      txt <- c(
        paste0("Delta=", round(st$mean_shBKCa[1] - st$mean_CTRL[1], 2)),
        paste0("W p/FDR=", fmt_p(st$wilcox_p[1]), "/", fmt_p(st$wilcox_padj[1]))
      )
      usr <- par("usr")
...
plot_gene_contribution_barplot <- function(contrib_df, file_path) {
  ...
  p <- ggplot(contrib_df, aes(x = gene_facet, y = delta, fill = delta)) +
    ...
    labs(
      ...
      subtitle = "Bars show the mean log2(count + 1) difference between shBKCa and CTRL",
      x = NULL,
...
plot_kcnma1_marker_summary_matrix <- function(summary_mat, file_path, main) {
  nr <- nrow(summary_mat)
  nc <- ncol(summary_mat)
...
plot_key_scatter_panels <- function(expr_mat, phenotype, pairs_df, file_path) {
  ...
  x <- expr_mat["KCNMA1", phenotype$sample]
  ...
  for (g in genes) {
    ...
    plot(x, y, pch = 21, bg = cols, col = "black", cex = 1.2,
         xlab = "KCNMA1 log2(count + 1)", ylab = paste0(g, " log2(count + 1)"),
         main = g)
...
plot_radial_kcnma1_network <- function(edge_df, node_categories, file_path) {
  if (nrow(edge_df) == 0) return(invisible(NULL))
  node_df <- data.frame(gene = c("KCNMA1", edge_df$gene_symbol), stringsAsFactors = FALSE)
  node_df <- node_df[!duplicated(node_df$gene), , drop = FALSE]
  outer_nodes <- node_df$gene[node_df$gene != "KCNMA1"]
  angles <- seq(0, 2 * pi, length.out = length(outer_nodes) + 1)[-1]
  coords <- data.frame(
    gene = c("KCNMA1", outer_nodes),
    x = c(0, cos(angles)),
    y = c(0, sin(angles)),
    stringsAsFactors = FALSE
  )
  coords$category <- node_categories$category[match(coords$gene, node_categories$gene_symbol)]
  coords$category[is.na(coords$category)] <- "BKCa"
  ...
  plot(NA, xlim = c(-1.35, 1.35), ylim = c(-1.35, 1.35), xaxt = "n", yaxt = "n",
       xlab = "", ylab = "", bty = "n",
       main = "BKCa-marker correlation network\n(overall Spearman across 8 samples; exploratory)")
  ...
  points(coords$x, coords$y, pch = 21,
         ...
         cex = ifelse(coords$gene == "KCNMA1", 3.2, 2.2))
...
plot_gsea_bubble <- function(gsea_res, file_path) {
  ...
  df$direction <- ifelse(df$NES > 0, "Enriched in shBKCa", "Enriched in CTRL")
  df$pathway_label <- gsub("^HALLMARK_", "", df$pathway)
  df$pathway_label <- factor(df$pathway_label, levels = df$pathway_label[order(df$NES)])
  p <- ggplot(df, aes(x = NES, y = pathway_label, fill = direction, size = -log10(pmax(padj, 1e-8)))) +
    geom_point(shape = 21, colour = "black", stroke = 0.25) +
    geom_vline(xintercept = 0, linetype = 2, linewidth = 0.35, colour = "grey55") +
    scale_fill_manual(
      values = c("Enriched in CTRL" = ctrl_color, "Enriched in shBKCa" = sh_color),
      name = NULL
    ) +
    scale_size_continuous(name = expression(-log[10]~"(FDR)"), range = c(2.2, 8)) +
    labs(
      title = "Hallmark GSEA - shBKCa vs CTRL",
    ...
    theme_stage(base_size = 11) +
    theme(
      axis.text.x = element_text(angle = 0, hjust = 0.5),
      axis.text.y = element_text(size = 8),
      legend.title = element_text(face = "bold")
    )
  save_plot(p, file_path, 8.5, max(6, 0.22 * nrow(df) + 2))
}
...
message0("=== BKCa / shBKCa bulk RNA-seq analysis ===")
if (!file.exists(input_file)) stop("Input file not found: ", input_file)

df0 <- utils::read.csv(input_file, check.names = FALSE, stringsAsFactors = FALSE)

# Sample columns are inferred from the original naming convention. This keeps
# the script reproducible without manual column selection.
sh_cols <- grep("^sushi66", colnames(df0), value = TRUE)
ctrl_cols <- grep("^sushishctrl", colnames(df0), value = TRUE)
sample_cols <- c(ctrl_cols, sh_cols)

if (length(sh_cols) == 0 || length(ctrl_cols) == 0) stop("Unable to identify CTRL/shBKCa sample columns.")

# Minimal sample metadata used throughout the analysis. Replicate IDs are
# derived from sample names because no separate metadata file is provided.
phenotype <- data.frame(
  sample = sample_cols,
  condition = factor(condition_from_sample(sample_cols), levels = c("CTRL", "shBKCa")),
  replicate = replicate_from_sample(sample_cols),
  stringsAsFactors = FALSE
)
...
# Standardize gene symbols, remove missing identifiers, and collapse duplicate
# symbols by mean expression to obtain one row per gene.
df0$gene_symbol <- toupper(trimws(as.character(df0$Gene.name)))
df0$gene_symbol[df0$gene_symbol %in% c("", "NA", "NAN", "NULL")] <- NA_character_
df0$gene_symbol[is.na(df0$gene_symbol)] <- sub("\\..*", "", as.character(df0$ID[is.na(df0$gene_symbol)]))
df0 <- df0[!is.na(df0$gene_symbol) & df0$gene_symbol != "", , drop = FALSE]

expr_raw <- as.matrix(df0[, sample_cols, drop = FALSE])
mode(expr_raw) <- "numeric"
rownames(expr_raw) <- df0$gene_symbol
expr_raw <- collapse_matrix_by_mean(expr_raw, rownames(expr_raw))
expr_log <- log2(expr_raw + 1)

# Build a clean DE result table from the upstream analysis so that downstream
# plots and summaries all use the same harmonized gene representation.
res_df <- data.frame(
  ID = as.character(df0$ID),
  gene_symbol = toupper(df0$gene_symbol),
  log2FoldChange = safe_num(df0$log2FoldChange),
  pvalue = safe_num(df0$pvalue),
  padj = safe_num(df0$padj),
  stringsAsFactors = FALSE
)
res_df <- collapse_res_table(res_df)
res_df$mean_CTRL <- rowMeans(expr_log[res_df$gene_symbol, ctrl_cols, drop = FALSE], na.rm = TRUE)
res_df$mean_shBKCa <- rowMeans(expr_log[res_df$gene_symbol, sh_cols, drop = FALSE], na.rm = TRUE)
res_df$mean_expression <- rowMeans(expr_log[res_df$gene_symbol, sample_cols, drop = FALSE], na.rm = TRUE)
res_df$delta_sh_minus_ctrl <- res_df$mean_shBKCa - res_df$mean_CTRL
...
marker_core_map <- data.frame(
  ...
  category = ifelse(core_marker_panel %in% c("ACTA2", "TAGLN", "COL1A1", "POSTN"), "myCAF",
             ifelse(core_marker_panel %in% c("IL6", "CXCL8", "CXCL12"), "iCAF",
             ...
             ifelse(core_marker_panel %in% c("RSAD2"), "ifCAF", "BKCa"))))),
  stringsAsFactors = FALSE
)
...
marker_category_map <- dplyr::bind_rows(
  data.frame(gene_symbol = "KCNMA1", category = "BKCa", stringsAsFactors = FALSE),
  marker_core_map,
...
corr_mat <- stats::cor(expr_log, method = "pearson", use = "pairwise.complete.obs")
...
kcnma1_expr <- if ("KCNMA1" %in% rownames(expr_log)) expr_log["KCNMA1", phenotype$sample] else rep(NA_real_, nrow(phenotype))

# At the marker level, the goal is to quantify both differential behavior
# between conditions and monotonic association with the perturbed channel gene.
marker_stats <- do.call(rbind, lapply(all_marker_genes, function(g) {
  ...
  sh_vals <- expr_vec[phenotype$condition == "shBKCa"]
  wt <- safe_wilcox_p(expr_vec, phenotype$condition)
  cor_all <- safe_spearman(kcnma1_expr, expr_vec)
  cor_ctrl <- safe_spearman(kcnma1_expr[phenotype$condition == "CTRL"], ctrl_vals)
  cor_sh <- safe_spearman(kcnma1_expr[phenotype$condition == "shBKCa"], sh_vals)
  data.frame(
    gene_symbol = g,
    category = marker_category_map$category[match(g, marker_category_map$gene_symbol)],
    mean_CTRL = mean(ctrl_vals, na.rm = TRUE),
    mean_shBKCa = mean(sh_vals, na.rm = TRUE),
    median_CTRL = stats::median(ctrl_vals, na.rm = TRUE),
    median_shBKCa = stats::median(sh_vals, na.rm = TRUE),
    ...
    rho_shBKCa = cor_sh["rho"],
    rho_shBKCa_pvalue = cor_sh["pvalue"],
    stringsAsFactors = FALSE
  )
}))
...
signature_stats <- do.call(rbind, lapply(colnames(score_mat), function(sig) {
  ...
  data.frame(
    ...
    mean_shBKCa = mean(x[phenotype$condition == "shBKCa"], na.rm = TRUE),
    median_CTRL = stats::median(x[phenotype$condition == "CTRL"], na.rm = TRUE),
    median_shBKCa = stats::median(x[phenotype$condition == "shBKCa"], na.rm = TRUE),
...
score_condition_mat <- rbind(
  ...
  shBKCa = colMeans(score_mat[phenotype$condition == "shBKCa", , drop = FALSE], na.rm = TRUE)
)
...
sh_mean <- rowMeans(contrib_mat[, phenotype$condition == "shBKCa", drop = FALSE], na.rm = TRUE)
...
###############################################################################
# 9. Summarize BKCa-centered marker correlations
###############################################################################

kcnma1_corr_focus <- marker_stats[marker_stats$gene_symbol != "KCNMA1", , drop = FALSE]
kcnma1_corr_focus <- kcnma1_corr_focus[order(kcnma1_corr_focus$rho_padj, -abs(kcnma1_corr_focus$rho_all)), , drop = FALSE]

summary_matrix_genes <- unique(c(
  head(kcnma1_corr_focus$gene_symbol[kcnma1_corr_focus$rho_all > 0], 8),
  head(kcnma1_corr_focus$gene_symbol[kcnma1_corr_focus$rho_all < 0], 8)
))
summary_matrix_genes <- summary_matrix_genes[summary_matrix_genes %in% kcnma1_corr_focus$gene_symbol]
summary_matrix <- cbind(
  rho_all = kcnma1_corr_focus$rho_all[match(summary_matrix_genes, kcnma1_corr_focus$gene_symbol)],
  delta_sh_minus_ctrl = kcnma1_corr_focus$delta_sh_minus_ctrl[match(summary_matrix_genes, kcnma1_corr_focus$gene_symbol)],
  log2FoldChange = kcnma1_corr_focus$log2FoldChange[match(summary_matrix_genes, kcnma1_corr_focus$gene_symbol)]
)
...
signature_blocks <- list(
  BKCa = intersect(c("KCNMA1"), rownames(expr_log)),
  myCAF = signature_list$myCAF_top30,
  iCAF = signature_list$iCAF_top30,
  csCAF = signature_list$csCAF_top30,
  apCAF = signature_list$apCAF_core,
...
annotation_col_block <- data.frame(
  Condition = factor(phenotype$condition, levels = c("CTRL", "shBKCa")),
  row.names = phenotype$sample,
  stringsAsFactors = FALSE
)
ann_colors_block <- list(
  Condition = c(CTRL = ctrl_color, shBKCa = sh_color)
)
...
combined_blocks <- c("BKCa", "myCAF", "iCAF", "csCAF", "ifCAF")
...
focus_blocks <- c("myCAF", "iCAF", "ifCAF")
row_order_focus <- unlist(row_orders[focus_blocks], use.names = FALSE)
if (length(row_order_focus) > 0) {
  focus_mat <- t(apply(expr_log[row_order_focus, phenotype$sample, drop = FALSE], 1, zscore_vec))
  focus_meta <- row_meta[row_meta$gene_symbol %in% row_order_focus, , drop = FALSE]
  focus_meta <- focus_meta[match(row_order_focus, focus_meta$gene_symbol), , drop = FALSE]
  rownames(focus_mat) <- focus_meta$label
  sample_order <- phenotype$sample[order(factor(phenotype$condition, levels = c("CTRL", "shBKCa")), phenotype$sample)]
  focus_display <- t(focus_mat[, sample_order, drop = FALSE])
  focus_lengths <- vapply(row_orders[focus_blocks], length, integer(1))
  focus_lengths <- focus_lengths[focus_lengths > 0]
  gaps_col_focus <- cumsum(focus_lengths)
  gaps_col_focus <- gaps_col_focus[gaps_col_focus < ncol(focus_display)]
  focus_annotation_row <- data.frame(
    Condition = factor(phenotype$condition[match(rownames(focus_display), phenotype$sample)], levels = c("CTRL", "shBKCa")),
    row.names = rownames(focus_display),
    stringsAsFactors = FALSE
  )
  gaps_row_focus <- cumsum(as.numeric(table(focus_annotation_row$Condition)))
  gaps_row_focus <- gaps_row_focus[gaps_row_focus < nrow(focus_display)]
  focus_ann_colors <- list(
    Condition = c(CTRL = ctrl_color, shBKCa = sh_color)
  )
  save_replicate_gene_landscape_heatmap(
    display_mat = focus_display,
    annotation_row = focus_annotation_row,
    annotation_colors = focus_ann_colors,
    gaps_row = gaps_row_focus,
    gaps_col = gaps_col_focus,
    file_path = file.path(fig_dir, "CAF_signature_heatmap_myCAF_iCAF_ifCAF_landscape.pdf"),
    width = 12.5,
    height = 4.4,
    title_text = "myCAF / iCAF / ifCAF signature heatmap"
  )
  save_replicate_gene_landscape_heatmap(
    display_mat = focus_display,
    annotation_row = focus_annotation_row,
    ...
    gaps_row = gaps_row_focus,
    gaps_col = gaps_col_focus,
    file_path = file.path(fig_dir, "CAF_signature_heatmap_myCAF_iCAF_ifCAF_landscape.png"),
    width = 12.5,
    height = 4.4,
    title_text = "myCAF / iCAF / ifCAF signature heatmap"
  )
...
if (length(summary_matrix_genes) >= 3) {
  kcnma1_pca_genes <- unique(c("KCNMA1", head(summary_matrix_genes, 12)))
  ...
  pca_kcnma1_df <- plot_pca(
    ...
    phenotype,
    file.path(fig_dir, "PCA_KCNMA1_module.pdf")
  )
} else {
  pca_kcnma1_df <- NULL
}
...
# Hallmark programs shift after BKCa perturbation.
...
expr_summary <- data.frame(
  metric = c("n_input_rows", "n_gene_symbols", "n_collapsed_genes", "n_samples", "n_ctrl", "n_shBKCa",
             "n_deg_padj_lt_0_05", "n_up_shBKCa", "n_down_shBKCa"),
  value = c(
    nrow(df0),
...
sample_summary_df <- sample_summary_stats(expr_log)
sample_summary_df$condition <- phenotype$condition[match(sample_summary_df$sample, phenotype$sample)]
sample_summary_df$KCNMA1 <- if ("KCNMA1" %in% rownames(expr_log)) as.numeric(expr_log["KCNMA1", sample_summary_df$sample]) else NA_real_
...
utils::write.csv(kcnma1_corr_focus, file.path(tab_dir, "KCNMA1_marker_correlations.csv"), row.names = FALSE, na = "")
...
if (exists("pca_kcnma1_df") && !is.null(pca_kcnma1_df)) {
  utils::write.csv(pca_kcnma1_df, file.path(tab_dir, "PCA_KCNMA1_module_coordinates.csv"), row.names = FALSE, na = "")
}
...
saveRDS(
  list(
    ...
    pca_kcnma1 = if (exists("pca_kcnma1_df")) pca_kcnma1_df else NULL
  ),
  file.path(rds_dir, "bkca_shbkca_bulk_analysis_bundle.rds")
)
...
best_pos <- marker_stats[marker_stats$gene_symbol != "KCNMA1" & marker_stats$rho_all > 0, , drop = FALSE]
best_neg <- marker_stats[marker_stats$gene_symbol != "KCNMA1" & marker_stats$rho_all < 0, , drop = FALSE]
...
best_sig <- signature_stats[order(signature_stats$wilcox_padj, -abs(signature_stats$mean_shBKCa - signature_stats$mean_CTRL)), , drop = FALSE]
...
readme_lines <- c(
  ...
  "",
  "## Summary",
  "This folder contains a cleaned and reproducible downstream analysis of the BKCa/KCNMA1 perturbation bulk RNA-seq dataset, centered on essential QC, CAF-oriented transcriptional programs, and the combined CAF heatmap without apCAF.",
  "",
  "## Key points",
  paste0("- collapsed genes: ", nrow(expr_log)),
  paste0("- samples: ", ncol(expr_log), " (", length(ctrl_cols), " CTRL, ", length(sh_cols), " shBKCa)"),
  ...
  paste0("- strongest positive `KCNMA1` correlation: `", best_pos$gene_symbol[1], "` (rho=", round(best_pos$rho_all[1], 3), ", FDR=", fmt_p(best_pos$rho_padj[1]), ")"),
  paste0("- strongest negative `KCNMA1` correlation: `", best_neg$gene_symbol[1], "` (rho=", round(best_neg$rho_all[1], 3), ", FDR=", fmt_p(best_neg$rho_padj[1]), ")"),
  ...
  "- `tables/KCNMA1_marker_correlations.csv`",
...
message0("BKCa bulk RNA-seq analysis complete.")
